...
Mirrors the OpenCode HTTP surface (server-mode AI coding tools
...
class KilocodeClient:
    ...
    def __init__(
        ...
        self.base_url = (base_url or settings.kilocode.base_url).rstrip("/")
        self.username = username or settings.kilocode.username
        self.password = password or settings.kilocode.password
        self.timeout = timeout or settings.kilocode.timeout_seconds
    ...
    async def send_message(
        ...
        Args:
            session_id: Kilocode session ID
            prompt: User prompt text
            model: Full model reference (e.g., "kilocode/some-model")
        """
        client = self._get_client()
        body: dict[str, Any] = {
        ...
        if model:
            # Kilocode expects model as object: { providerID, modelID }
            parts = model.split("/", 1)
            if len(parts) == 2:
                body["model"] = {
                    "providerID": parts[0],
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Failed to delete Kilocode session",
                session_id=session_id,
                error=str(exc),
            )
            return False

    async def __aenter__(self) -> "KilocodeClient":
        return self
