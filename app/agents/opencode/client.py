...
class OpenCodeClient:
    ...
    def __init__(
        ...
        self.timeout = timeout or settings.opencode.timeout_seconds
    ...
    async def send_message(
        ...
        if model:
            # OpenCode expects model as object: { providerID, modelID }
            parts = model.split("/", 1)
            if len(parts) == 2:
                body["model"] = {
                    "providerID": parts[0],
                ...
                # Fallback: treat whole string as modelID with no provider
                body["model"] = {
                    "providerID": "",
                    "modelID": model,
                }
